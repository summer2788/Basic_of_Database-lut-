...
def searchPlayer():
    ...
    print("Last name:", results[2])
    print("Birthdate: ", results[4])
    print("Nationality:", results[3])
...
def moveMatch():
    ...
    if (newMatchDate == 'NULL'):
        cur.execute(
            "UPDATE Matches SET matchdate =?,resultSets=?,winnerID=? WHERE matchid=?;", (None, None, None, matchID))
    else:
        cur.execute("UPDATE Matches SET matchdate =? WHERE matchid=?;",
...
def deletePlayer():
    ...
    cur.execute(
        "UPDATE Matches SET FK_playerOne =? WHERE FK_playerOne=?;", (None, playerID))
    cur.execute(
        "UPDATE Matches SET FK_playerTwo =? WHERE FK_playerTwo=?;", (None, playerID))
    cur.execute(
        "UPDATE Matches SET winnerID =? WHERE winnerID=?;", (None, playerID))
